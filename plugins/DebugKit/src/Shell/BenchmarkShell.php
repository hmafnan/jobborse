...
class BenchmarkShell extends Shell
{
    ...
    public function main()
    {
        ...
        $this->out(Text::insert(__d('DebugKit', '-> Testing :url'), compact('url')));
        $this->out("");
    ...
    protected function _results($times)
    {
        ...
        $this->out(Text::insert(__d('DebugKit', 'Total Requests made: :requests'), compact('requests')));
        $this->out(Text::insert(__d('DebugKit', 'Total Time elapsed: :duration (seconds)'), compact('duration')));

        $this->out("");

        $this->out(Text::insert(__d('DebugKit', 'Requests/Second: :rps req/sec'), [
                'rps' => round($requests / $duration, 3)
        ]));

        $this->out(Text::insert(__d('DebugKit', 'Average request time: :average-time seconds'), [
                'average-time' => round($duration / $requests, 3)
        ]));

        $this->out(Text::insert(__d('DebugKit', 'Standard deviation of average request time: :std-dev'), [
                'std-dev' => round($this->_deviation($times, true), 3)
        ]));

        $this->out(Text::insert(__d('DebugKit', 'Longest/shortest request: :longest sec/:shortest sec'), [
    ...
    public function getOptionParser()
    {
        ...
        ->addArgument('url', [
            'help' => __d('DebugKit', 'The URL to request.'),
            'required' => true
        ])
        ->addOption('n', [
            'default' => 10,
            'help' => __d('DebugKit', 'Number of iterations to perform.')
        ])
        ->addOption('t', [
            'default' => 100,
            'help' => __d(
