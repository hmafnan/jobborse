    ...
    public function implementedEvents()
    {
        $before = function ($name) {
            return function () use ($name) {
                DebugTimer::start($name, __d('DebugKit', $name));
            };
        };
        $after = function ($name) {
            return function () use ($name) {
        ...
        return [
            'Controller.initialize' => [
                ['priority' => 0, 'callable' => function () {
                    DebugMemory::record(__d('DebugKit', 'Controller initialization'));
                }],
            ...
            'Controller.startup' => [
                ...
                ['priority' => 999, 'callable' => function () {
                    DebugMemory::record(__d('DebugKit', 'Controller action start'));
                    DebugTimer::start(__d('DebugKit', 'Controller action'));
                }],
            ],
            'Controller.beforeRender' => [
                ['priority' => 0, 'callable' => function () {
                    DebugTimer::stop(__d('DebugKit', 'Controller action'));
                }],
                ['priority' => 0, 'callable' => $before('Event: Controller.beforeRender')],
                ['priority' => 999, 'callable' => $after('Event: Controller.beforeRender')],
                ['priority' => 999, 'callable' => function () {
                    DebugMemory::record(__d('DebugKit', 'View Render start'));
                    DebugTimer::start(__d('DebugKit', 'View Render start'));
                }],
            ],
            ...
            'View.beforeRenderFile' => [
                ['priority' => 0, 'callable' => function ($event, $filename) {
                    DebugTimer::start(__d('DebugKit', 'Render {0}', $filename));
                }],
            ],
            'View.afterRenderFile' => [
                ['priority' => 0, 'callable' => function ($event, $filename) {
                    DebugTimer::stop(__d('DebugKit', 'Render {0}', $filename));
                }],
            ],
            'Controller.shutdown' => [
                ['priority' => 0, 'callable' => $before('Event: Controller.shutdown')],
                ['priority' => 0, 'callable' => function () {
                    DebugTimer::stop(__d('DebugKit', 'View Render start'));
                    DebugMemory::record(__d('DebugKit', 'Controller shutdown'));
                }],
