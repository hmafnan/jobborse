...
<?php if (empty($metrics)): ?>
    <p class="info"><?= __d('DebugKit', 'There were no cache operations this request.') ?></p>
<?php else: ?>
    <?php foreach ($metrics as $name => $counters): ?>
    <section class="section-tile">
        <h3><?= __d('DebugKit', '{0} Metrics', h($name)) ?> </h3>
        ...
        <table cellspacing="0" cellpadding="0" class="debug-table">
            <thead>
                <tr>
                    <th><?= __d('DebugKit', 'Metric') ?></th>
                    <th><?= __d('DebugKit', 'Total') ?></th>
                </tr>
            </thead>
            <tbody>
