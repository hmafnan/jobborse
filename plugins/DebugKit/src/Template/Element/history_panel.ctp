...
<?php if (empty($requests)): ?>
    <p class="warning"><?= __d('DebugKit', 'No previous requests logged.') ?></p>
<?php else: ?>
    <p><?= count($requests) ?> <?= __d('DebugKit', 'previous requests available') ?></p>
    <ul class="history-list">
        <li>
            <?= $this->Html->link(
                __d('DebugKit', 'Back to current request'),
