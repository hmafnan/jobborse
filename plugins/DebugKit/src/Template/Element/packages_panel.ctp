...
<?php if (empty($packages) && empty($devPackages)): ?>
    <div class="warning">
        <?= __d('DebugKit', '{0} not found', "'composer.lock'"); ?>
    </div>
<?php else: ?>
    <div class="check-update">
        <button class="btn-primary">Check for Updates</button>
        <label><input type="checkbox" class="direct-dependency"><?= __d('DebugKit', 'Direct dependencies only') ?></label>
    </div>
    <div class="terminal"></div>
    <?php if (!empty($packages)): ?>
        <section class="section-tile">
            <h3><?= __d('DebugKit', 'Requirements ({0})', count($packages)) ?> </h3>
    ...
    <?php if (!empty($devPackages)): ?>
        <section class="section-tile">
            <h3><?= __d('DebugKit', 'Dev Requirements ({0})', count($devPackages)) ?> </h3>
            <table cellspacing="0" cellpadding="0" class="debug-table">
                <thead>
                <tr>
                    <th>Name</th>
